...
describe('Update Product Service Tests', () => {
  ...
  describe('deve atualizar um produto com sucesso', () => {
    it('should update a product successfully', async () => {
      ...
      const mockProductRepository: IProductRepository = {
        update: updateMock,
        getAllByName: jest.fn().mockResolvedValue([existentProduct]), // Simula que não há conflito de nome com outro produto
        findById: jest.fn().mockResolvedValue(existentProduct), // Simula que o produto a ser atualizado existe
  ...
  describe('deve falhar ao atualizar um produto porque um produto com o mesmo nome já existe', () => {
    it('should fail in update a product because a product with the same name already exists', async () => {
      ...
      const updateProductDto = new UpdateProductDto();
      updateProductDto.name = 'Existing Product Name'; // Nome que já existe em outro produto
